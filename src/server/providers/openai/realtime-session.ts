...
export async function createRealtimeClientSecret(requestedVoice?:string){
 ...
 const response=await fetch("https://api.openai.com/v1/realtime/client_secrets",{
  ...
  body:JSON.stringify({session:{
   ...
   audio:{input:{turn_detection:{type:"server_vad",threshold:0.5,prefix_padding_ms:300,silence_duration_ms:650,create_response:true,interrupt_response:true}},output:{voice:config.voice}}
  }})
 });
